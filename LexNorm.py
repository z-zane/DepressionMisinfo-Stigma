...
@File: LexNorm.py
@Author: Zane Zhang
@Date:  2025/06/04 17:47:12
@Description: 
    In this script, we define a additional function, replace_custom_errors, to correct some spelling mistakes which the LexNorm cannot handle well.
    
    And we annotate main print to avoid interrupt main function.
    
    Original version at https://github.com/AnneDirkson/LexNorm
"""
...
#         tokens2 = [t[0] for t in tokens]
        ...
        for t in tokens:
            ...
            if t_lower not in IGNORE_LIST:
                ...
                #URLS
                elif re.search(r'http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+', t_lower) and ignore_urls:
                    IGNORE_LIST.append(t_lower)
                    text_string += t_lower + ' '                
...
#                         self.replaced_with.append(correct)
    ...
    def replace_custom_errors(self, posts):
        '''
        custom spelling mistakes replacement 
        for incomplete ability to detect some spellings which reported in test_normalizer file, thus add a function to solve them
        '''
        replacements = {
            'iving': 'living',
            'wanna': 'want to',
...
#--------Overall normalization function--------------------------------------
    ...
    def normalize (self, posts, anonymize = True, remove_foreign = False):
        ...
        # replace custom errors
        posts4 = self.replace_custom_errors(posts4)
